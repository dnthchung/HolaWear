...
export class SuccessResponse<T = any> {
  ...
  constructor(message: string = 'success', statusCode: number = 200, data?: T, metadata?: ISuccessResponse<T>['metadata']) {
    this.response = {
      status: 'success',
      statusCode,
      message,
...
export class ForbiddenError extends ErrorResponse {
  constructor(message: string = 'Server: Forbidden') {
    super(message, 403, 'FORBIDDEN')
  }
}

export class NotFoundError extends ErrorResponse {
  constructor(message: string = 'Server: Resource not found') {
    super(message, 404, 'NOT_FOUND')
  }
}

export class ConflictError extends ErrorResponse {
  constructor(message: string = 'Server: Resource conflict', details?: any) {
...
export class ValidationError extends ErrorResponse {
  constructor(message: string = 'Server: Validation failed', details?: any) {
...
export class InternalServerError extends ErrorResponse {
  constructor(message: string = 'Server: Internal server error') {
    super(message, 500, 'INTERNAL_SERVER_ERROR')
  }
}

export class TooManyRequest extends ErrorResponse {
  constructor(message: string = 'Server: Too many requests') {
