      ...
      <table className="min-w-full table-auto">
        <thead>
          <tr className="border-b">
            <th className="text-left py-2 px-4">ColorId</th>
        ...
        <tbody>
          {currentProducts.map((product, index) => (
            <tr key={index} className="border-b hover:bg-gray-50">
              <td className="py-2 px-4">{product.colorId}</td>
              <td className="py-2 px-4 flex items-center">
                <img src={product.imageUrl} alt={product.colorName} className="w-8 h-8 rounded mr-2" />
                {product.colorName}
              </td>
