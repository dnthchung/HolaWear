      ...
      <table className="min-w-full table-auto">
        <thead>
          <tr className="border-b">
            <th className="text-left py-2 px-4">TypeId</th>
        ...
        <tbody>
          {currentProducts.map((product, index) => (
            <tr key={index} className="border-b hover:bg-gray-50">
              <td className="py-2 px-4">{product.typeId}</td>
              <td className="py-2 px-4 flex items-center">
                <img src={product.imageUrl} alt={product.typeName} className="w-8 h-8 rounded mr-2" />
                {product.typeName}
              </td>
